...
class StableDiffusionPipelineOutputWithNoise(BaseOutput):
    images: np.ndarray
    nsfw_content_detected: List[bool]
    noise: Union[List[PIL.Image.Image], np.ndarray]
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(vec_t, torch.Tensor) and vec_t.ndim == 0:
        ...
        if not return_dict:
            return (image, has_nsfw_concept,noise)

        return StableDiffusionPipelineOutputWithNoise(images=image, nsfw_content_detected=has_nsfw_concept,noise=noise)




class RectifiedFlowPipelineWithVar(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    r"""
    Pipeline for text-to-image generation using Rectified Flow and Euler discretization.
    This customized pipeline is based on StableDiffusionPipeline from the official Diffusers library (0.21.4)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(vec_t, torch.Tensor) and vec_t.ndim == 0:
                    vec_t = vec_t.unsqueeze(0)
                result=self.unet(latent_model_input, vec_t, encoder_hidden_states=prompt_embeds)
                var = torch.clamp(result.variance, min=1e-6)
                std = torch.sqrt(var)
                v_pred = result.sample + std * torch.randn_like(result.sample)
